...
static char* myRegion = regions[1] ; // variavel char* criada para  apontar para a String "Au915"
static uint8_t global_region = 1;
/*!
 * User application data structure
 */
lora_AppData_t AppData={ AppDataBuff,  0 ,0 }; // criação padrão par envio de dados via lora, valor maximo que será enviado, tamanhõ da menssagem enviada, porta para envio
...
static void LORA_ConfirmClass ( DeviceClass_t CLASS_A ); // DeviceClass_t é um Enum que possui as 3 classifiaçãoes de classe lora, metodo chamando essa class e usando a Class_A
...
static TimerEvent_t TxTimer; // configuração de tempo para evento ?
...
void LORAWAN_init(uint8_t region){
	...
	//SX126xReset();
	//sets LoRaWAN region
	 setRegion(1); // aponta para o valor 1 da posição de au915 na matraiz
...
static void OnTxTimerEvent( void* context ){
	...
	appData.Buff = (uint8_t*)"teste lora "; // o que vai ser enviado
	appData.BuffSize = strlen((const char*)appData.Buff); // convertando a string que vai ser enviada em binário
	appData.Port = LORAWAN_APP_PORT; // por que vai ser enviada
	
	
	lorawan_send(&appData); // & = endereço de memória dos dados que vamo ser enviados
